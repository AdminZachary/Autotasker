...
| 用例编号 | 用例名称 | 参与者 | 简要说明 |
| --- | --- | --- | --- |
| UC-01 | 注册/登录系统 | 用户 | 用户进入系统并建立个人账户 |
| UC-02 | 设置个人偏好 | 用户 | 用户设置休息时间和专注时长 |
| UC-03 | 输入目标 | 用户 | 用户输入需要完成的宏观目标 |
| UC-04 | 获取任务草案 | 用户 | 系统分析目标并生成任务草案 |
| UC-05 | 修改并确认任务 | 用户 | 用户修改草案并生成正式任务 |
| UC-06 | 查看看板 | 用户 | 用户查看任务状态和进度 |
| UC-07 | 更新任务状态 | 用户 | 用户更新任务为待办、进行中或已完成 |
| UC-08 | 启动番茄钟 | 用户 | 用户借助番茄钟执行任务 |
| UC-09 | 查看统计与复盘 | 用户 | 用户查看图表和建议 |
| UC-10 | 维护系统配置 | 管理员 | 管理员维护系统运行参数 |
